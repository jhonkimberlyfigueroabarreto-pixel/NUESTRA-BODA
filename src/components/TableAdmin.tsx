...
export default function TableAdmin() {
  ...
  const saveTablesToStorage = async (updatedTables: Table[]) => {
    ...
    try {
      ...
      const updatePromises = parsedGuests.map(async (guest) => {
  ...
  const moveGuest = (guestName: string, sourceId: string | "bank", targetId: string | "bank") => {
    ...
    // Get quota of moving guest
    const guestObj = allGuests.find(g => `${g.firstName} ${g.lastName}` === guestName);
    const guestQuota = guestObj ? guestObj.quota : 1;

    // Calculate current occupied cupos in target table
    const targetOccupiedCupos = targetTable.guests.reduce((sum, gName) => {
      const g = allGuests.find(g => `${g.firstName} ${g.lastName}` === gName);
      return sum + (g ? g.quota : 1);
    }, 0);

    const projectedOccupancy = targetOccupiedCupos + guestQuota;
    const capacity = targetTable.capacity || 10;

    if (projectedOccupancy > capacity) {
      alert("No es posible asignar este invitado a la mesa seleccionada porque la capacidad sería superada.");
      return;
    }
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div className="xl:col-span-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {tables.map((table) => {
              const occupiedCupos = table.guests.reduce((sum, gName) => {
                const g = allGuests.find(g => `${g.firstName} ${g.lastName}` === gName);
                return sum + (g ? g.quota : 1);
              }, 0);
              const capacity = table.capacity || 10;
              const available = capacity - occupiedCupos;
              const isOverCapacity = occupiedCupos > capacity;
              const percent = Math.min((occupiedCupos / capacity) * 100, 100);
                ...
                >
                  ...
                  <div className="px-4 py-2.5 bg-zinc-950/30 border-b border-zinc-850/60 flex flex-col gap-1.5 text-[11px]">
                    <div className="flex justify-between items-center text-zinc-400">
                      ...
                      <span className={`font-mono font-bold ${isOverCapacity ? "text-rose-400" : occupiedCupos === capacity ? "text-emerald-400" : "text-zinc-200"}`}>
                        {occupiedCupos} / {capacity} asientos
                      </span>
                    </div>
            ...
            >
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                ...
                {tables.map((t) => {
                  ...
                  const occupiedCupos = t.guests.reduce((sum, gName) => {
                    const g = allGuests.find(g => `${g.firstName} ${g.lastName}` === gName);
                    return sum + (g ? g.quota : 1);
                  }, 0);
                  const cap = t.capacity || 10;
                  const isFull = occupiedCupos >= cap;
